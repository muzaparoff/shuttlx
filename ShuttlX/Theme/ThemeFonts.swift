...
// Use `var f = ThemeFonts(); f.timerDisplay = myFont` to build from a preset.

struct ThemeFonts: Equatable {
    // Shared
    var timerDisplay: Font   = .system(.largeTitle, design: .monospaced).weight(.semibold)
    var metricLarge: Font    = .system(.largeTitle, design: .rounded).weight(.bold)
    var metricMedium: Font   = .system(.title2, design: .rounded).weight(.semibold)
    var metricSmall: Font    = .system(.body, design: .rounded).weight(.medium)
    ...
    // iOS-specific
    var heroIcon: Font       = .system(size: 48)
    var onboardingIcon: Font = .system(size: 72)
