...
class CustomWorkoutIntegrationTests: XCTestCase {
    ...
    func testWorkoutIntervalGeneration() throws {
        ...
        // Should have warmup
        let warmupIntervals = intervals.filter { $0.type == .warmup }
        XCTAssertEqual(warmupIntervals.count, 1)
        XCTAssertEqual(warmupIntervals.first?.duration, 300) // 5 minutes
        
        // Should have cooldown
        let cooldownIntervals = intervals.filter { $0.type == .cooldown }
        XCTAssertEqual(cooldownIntervals.count, 1)
        XCTAssertEqual(cooldownIntervals.first?.duration, 300) // 5 minutes
        ...
        print("   - Warmup: \(warmupIntervals.count), Work: \(workIntervals.count), Rest: \(restIntervals.count), Cooldown: \(cooldownIntervals.count)")
    ...
    func testCompleteCustomWorkoutFlow() throws {
        ...
        for (index, interval) in intervals.enumerated() {
            ...
            case .warmup:
                simulatedCalories += 5 // Low calorie burn during warmup
                simulatedDistance += 100 // Slow pace
                maxHeartRate = max(maxHeartRate, 110)
            case .work:
            ...
            case .cooldown:
                simulatedCalories += 3 // Low calorie burn during cooldown
                simulatedDistance += 80 // Slow pace
                maxHeartRate = max(maxHeartRate, 105)
            }
