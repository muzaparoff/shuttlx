...
class CriticalBugFixTests: XCTestCase {
    ...
    func testTimerStartsCriticalFix() throws {
        ...
        let testProgram = TrainingProgram(
            ...
            totalDuration: 5.0, // 5 minutes total
            difficulty: .beginner,
    ...
    func testTimerProgressesThroughIntervals() throws {
        ...
        let timer = Timer.scheduledTimer(withTimeInterval: 0.5, repeats: true) { timer in
            ...
            if checkCount == 2 { // After 1 second - should be in warmup
                XCTAssertEqual(self.workoutManager.currentInterval?.name, "Test Warmup")
                XCTAssertEqual(self.workoutManager.currentIntervalIndex, 0)
            }
            
            if checkCount == 6 { // After 3 seconds - should be in work interval
